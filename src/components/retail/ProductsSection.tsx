...
export default function ProductsSection() {
  ...
  useEffect(() => {
    ...
    const fetchData = async () => {
      ...
      try {
        ...
        // No shopType on the retail-level entity yet, so fall back to general category suggestions.
        const generalCategories = ['Food', 'Stationary', 'Kitchenware', 'Clothes', 'Beverages', 'Snacks'];
        setCategorySuggestions(generalCategories);
        setCategories(['All products', ...generalCategories]);
  ...
  const fetchProducts = async (shopId: number) => {
    ...
    setFetchError(null);
    try {
      const productsResponse = await authAPI.getRProductsByRShopID(shopId);
